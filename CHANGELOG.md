...
- **Localnet Docker images bumped to the current support-matrix generation.** The localnet was pinned to the older Mainnet row (`midnight-node:0.22.5` / `indexer-standalone:4.0.1` / `proof-server:8.0.3`), while the CLI's SDK stack (`midnight-js 4.1.1`, `ledger-v8 8.x`, `compact-runtime 0.16.0`, `dapp-connector-api 4.0.1`) already targets the newer **Preview/Preprod** row. Images now track that same row — `midnight-node 0.22.5 → 1.0.0`, `indexer-standalone 4.0.1 → 4.3.3`, `proof-server 8.0.3 → 8.1.0` — per the [Midnight support matrix](https://docs.midnight.network/relnotes/support-matrix), so the local chain runs the node generation the SDK is built for instead of trailing it. `COMPOSE_VERSION` bumped so existing `~/.midnight/localnet/compose.yml` files are regenerated on next `mn localnet up`. Verified end-to-end on a cold localnet: airdrop (genesis sync → dust → ZK proof via 8.1.0 → finalize on node 1.0.0) and balance both succeed.
  ...
  - Added `restart: on-failure` to the indexer service. The SPO sub-indexer queries block `#1` at startup and exits the whole indexer if it isn't there yet; on a fresh chain the node hasn't minted `#1` in the first few seconds, so the first launch loses that race. The restart lets it self-heal once block `#1` exists (~6 s block time) instead of leaving the indexer down.
